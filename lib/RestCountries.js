...
class RestCountries {
  ...
  constructor() {
    ...
    // Use localfile since restcountries.eu is not reachable anymore
    this.apiResponseFile = './lib/resources/api_response.json';
  }
  ...
  _performAsyncRequest(opt) {
    // https://restcountries.eu seems to be down since some weeks ago
    // doing this fix for keep this library up and running until the service
    // is available
    ...
    return new Promise((resolve, reject) => {
      fs.readFile(this.apiResponseFile, (err, data) => {
        if (err) {
          reject(err);
        } else {
